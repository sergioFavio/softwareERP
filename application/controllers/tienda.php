...
*/
	...
	public function verPedidosPorCliente(){
		...
		if($permisoUserName!='superuser' && $permisoUserName!='developer' && $permisoMenu!='ventas'){  //... valida permiso de userName y de menu ...
			...
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/verPedidosPorCliente';
	...
	public function buscarPedidoPorCliente(){
		//... buscar los registros que coincidan con el patron busqueda ingresado ...
		...
		//	redirect('produccion/crudVerCotizaciones');
			...
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/buscarPedidoPorCliente';
	...
	}		//... fin funcion: buscarPedidoPorCliente ...
...
//			redirect('menuController/index');
	...
	public function verProformas(){
		...
		if($permisoUserName!='superuser' && $permisoUserName!='developer' && $permisoMenu!='ventas' && $permisoMenu!='produccion'){  //... valida permiso de userName y de menu ...
			...
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/verProformas';
			
			/*Obtiene el total de registros a paginar */
	    	$config['total_rows'] = $this->tablaGenerica_model->get_total_registros('proformacabecera');
		
			$contador= $this->tablaGenerica_model->get_total_registros('proformacabecera'); //...contador de registros  ...		
			if($contador==0){
				$datos['mensaje']='No hay registros para mostrar ';
				$this->load->view('header');
				$this->load->view('mensaje',$datos );
				...
				$datos['consultaProforma'] ='';
				$datos['permisoUserName'] =$permisoUserName;
				$datos['buscarPor'] ='idProforma';
				
				/*Se llama a la vista para mostrar la información*/
				$this->load->view('header');
				$this->load->view('tienda/verProformas', $datos);
				$this->load->view('footer');
	...
	public function verProformasPorCliente(){
		//... control de permisos de acceso ....
		$permisoUserName=$this->session->userdata('userName');
		$permisoMenu=$this->session->userdata('usuarioMenu');
		$permisoProceso1=$this->session->userdata('usuarioProceso1');
		if($permisoUserName!='superuser' && $permisoUserName!='developer' && $permisoMenu!='ventas' && $permisoMenu!='produccion'){  //... valida permiso de userName y de menu ...
			...
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/verProformasPorCliente';
...
//		$archivoPDF='pedido'.$numePedidoSinGuion.'.pdf';
	...
	public function buscarProforma(){
		//... buscar los registros que coincidan con el patron busqueda ingresado ...

		$campo1='idProforma';   	//... el campo por elcual se va hacer la búsqueda ...
		
		$permisoUserName=$this->session->userdata('userName');
		if(isset($_POST['inputBuscarPatron'])){
			$consultaProforma=$_POST['inputBuscarPatron'];
			
			// Escribimos una primera línea en consultaCrud.txt
			$fp = fopen("pdfsArchivos/consultaCrud.txt", "w");
			fputs($fp, $consultaProforma);
			fclose($fp); 
		...
		$totalRegistrosEncontrados=0;		
		$totalRegistrosEncontrados=$this->tablaGenerica_model->getTotalRegistrosBuscar('proformacabecera',$campo1,$consultaProforma);
		//echo"total registros econtrados".$totalRegistrosEncontrados;
		if($totalRegistrosEncontrados==0){
			redirect('menuController/index');
		}else{
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/buscarProforma';
			
			/*Obtiene el total de registros a paginar */
	    	$config['total_rows'] = $this->tablaGenerica_model->getTotalRegistrosBuscar('proformacabecera',$campo1,$consultaProforma);
			...
			/* Se obtienen los registros a mostrar*/ 
			$datos['listaProforma'] = $this-> tablaGenerica_model -> buscarPaginacion('proformacabecera',$campo1,$consultaProforma, $config['per_page'], $desde );
			$datos['consultaProforma'] =$consultaProforma;
			$datos['permisoUserName'] =$permisoUserName;
			$datos['buscarPor'] ='idProforma';
	...
	}		//... fin funcion: buscarProforma ...
	
		
	public function buscarProformaPorCliente(){
		//... buscar los registros que coincidan con el patron busqueda ingresado ...

		$campo1='clienteProf';   	//... el campo por elcual se va hacer la búsqueda ...
		...
		//	redirect('produccion/crudVerCotizaciones');
			...
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/buscarProformaPorCliente';
	...
	}		//... fin funcion: buscarProformaPorCliente ...
	
	
	public function ubicarPedidoNotaEntrega(){
		//... control de permisos de acceso ....
		$permisoUserName=$this->session->userdata('userName');
		$permisoMenu=$this->session->userdata('usuarioMenu');
		if($permisoUserName!='superuser' && $permisoUserName!='developer' && $permisoMenu!='ventas' && $permisoMenu!='produccion'){  //... valida permiso de userName y de menu ...
			...
			$this->load->view('header');
			$this->load->view('tienda/buscarPedidoNotaEntrega',$datos);
			$this->load->view('footer');
		}		//... fin IF validar usuario ...
	}	//..fin ubicarPedidoNotaEntrega ...
	...
	public function grabarNotaEntrega(){
		...
		redirect("tienda/generarNotaEntregaPDF?numeroEntrega=$numEntrega&local=$local");
	
	}	//..fin funcion: grabarNotaEntrega ...
	
	
	public function generarNotaEntregaPDF(){
		//... genera reporte de salida en PDF
		$numeroEntrega= $_GET['numeroEntrega']; 		//... lee numeroEntrega que viene de grabarNotaEntrega ...
		...
		// Se carga la libreria fpdf
		$this->load->library('tienda/NotaEntregaPdf');
		
		// Se obtienen los registros de la base de datos
		if($local=='Z'){									//... cuando notaEntrega es Z:úñiga ...
			$sql ="SELECT numEntrega,pedidoItem,producto,cantidad,unidad FROM entregaproductoz WHERE numEntrega='$numeroEntrega' ";	
		}else{												//... cuando notaEntrega es T:tienda o F:fábrica ...
			$sql ="SELECT numEntrega,pedidoItem,producto,cantidad,unidad FROM entregaproducto WHERE numEntrega='$numeroEntrega' ";	
		}
		...
		if($local=='Z'){								//... cuando notaEntrega es Z:úñiga ...						
			$entregaCabecera= $this->tablaGenerica_model->buscar('entregacabeceraz','entrega',$numeroEntrega); //..carga el modelo de la tabla ..
		}else{											//... cuando notaEntrega es T:tienda o F:fábrica ...
			$entregaCabecera= $this->tablaGenerica_model->buscar('entregacabecera','entrega',$numeroEntrega); //..carga el modelo de la tabla ..
		}
		
		$fechaEntrega= $entregaCabecera["fecha"];		// ... forma de asignar cuando se utliza funcion ...buscar ... de tablaGenerica_model ...
		$pedido= $entregaCabecera["pedido"];			// ... forma de asignar cuando se utliza funcion ...buscar ... de tablaGenerica_model ...
		$cliente= $entregaCabecera["cliente"];			// ... forma de asignar cuando se utliza funcion ...buscar ... de tablaGenerica_model ...
		$direccion= $entregaCabecera["direccion"];		// ... forma de asignar cuando se utliza funcion ...buscar ... de tablaGenerica_model ...
		$fono= $entregaCabecera["telefono"];			// ... forma de asignar cuando se utliza funcion ...buscar ... de tablaGenerica_model ...
					
		if($local=='Z'){								//... cuando notaEntrega es Z:úñiga ...									
			$sql ="SELECT * FROM entregacabeceraz WHERE entrega='$numeroEntrega' ";
		}else{												//... cuando notaEntrega es T:tienda o F:fábrica ...
			$sql ="SELECT * FROM entregacabecera WHERE entrega='$numeroEntrega' ";
		}	
		...
		if($contador==0){
			$datos['mensaje']='No hay registro grabado con el n&uacute;mero de  entrega '.$numeroEntrega.' en la tabla ENTREGA CABECERA.';
			$this->load->view('header');
			$this->load->view('mensaje',$datos );
			$this->load->view('footer');
		}else{
		    ...
		    */
		    ...
		    ob_clean(); // cierra si es se abrio el envio de pdf...
		    $this->pdf = new NotaEntregaPdf();
			
			$this->pdf->local=$nombreLocal;   			   				//...pasando variable para el header del PDF
			$this->pdf->numeroEntrega=$numeroEntrega;				    //...pasando variable para el header del PDF
			$this->pdf->fechaEntrega=fechaMysqlParaLatina($fechaEntrega); 		//...pasando variable para el header del PDF
			$this->pdf->cliente=$cliente; 								//...pasando variable para el header del PDF
			$this->pdf->direccion=$direccion; 							//...pasando variable para el header del PDF
			$this->pdf->fonoCelular=$fono; 								//...pasando variable para el header del PDF
			
			$this->pdf->secuenciaEntrega=substr($numeroEntrega,0,4); 		//...pasando variable para el header del PDF
			$this->pdf->anhoSistema=substr($numeroEntrega,4,strlen($numeroEntrega)-4); 	//...pasando variable para el header del PDF
		    ...
		    */
			...
			$this->pdf->Output('pdfsArchivos/entregas/notaEntrega'.$numeroEntrega.'.pdf', 'F');
			 
			$datos['documento']="pdfsArchivos/entregas/notaEntrega".$numeroEntrega.".pdf";	
			$datos['titulo']=' Nota de Entrega No. '.substr($numeroEntrega,0,4).'/'.substr($numeroEntrega,4,strlen($numeroEntrega)-4);	// ... titulo ...
	...
	} //... fin funcion: generarNotaEntregaPDF ...
	
		
	public function verNotasEntrega(){
		//... control de permisos de acceso ....
		$permisoUserName=$this->session->userdata('userName');
		$permisoMenu=$this->session->userdata('usuarioMenu');
		$permisoProceso1=$this->session->userdata('usuarioProceso1');
		if($permisoUserName!='superuser' && $permisoUserName!='developer' && $permisoMenu!='ventas' && $permisoMenu!='produccion'){  //... valida permiso de userName y de menu ...
			...
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/verNotasEntrega';
	...
	public function entregaPdfCrud(){
		//... recupera la variable de numePedido ...
		$numeEntrega=$_POST["numeEntrega"];
		?>
		<embed src="<?= base_url('pdfsArchivos/entregas/notaEntrega'.$numeEntrega.'.pdf') ?>" width="820" height="455" id="sergio"> <!-- documento embebido PDF -->
		<?php
	}
	...
	public function buscarEntrega(){
		...
		if($totalRegistrosEncontrados==0){
			...
			/* URL a la que se desea agregar la paginación*/
	    	$config['base_url'] = base_url().'tienda/buscarEntrega';
