...
$sql="SELECT * FROM contaplandectas WHERE  cuenta='13000000' ";
$registros = $this->db->query($sql);
...
$sql="SELECT * FROM contaplandectas WHERE  cuenta='12000000' ";
$registros = $this->db->query($sql);
...
$totalOrigenFondos= $resultadoGestion + $previsiones + $depreciaciones + $ajusteCapital;
$this->pdf->Cell(63,5,'','',0,'L',0);
...
$sql="SELECT * FROM contaplandectas WHERE  cuenta='14000000' ";
$registros = $this->db->query($sql);
...
$sql="SELECT * FROM contaplandectas WHERE  cuenta='21020000' ";
$registros = $this->db->query($sql);
...
$sql="SELECT * FROM contaplandectas WHERE  cuenta='21010000' ";
$registros = $this->db->query($sql);
...
$totalAplicacionFondos= 99999999;
$this->pdf->Cell(63,5,'','',0,'L',0);
...
//							$this->pdf->Cell(20,5,number_format($registro->habermes,2),'',0,'R',0);
	...
	public function generarReporteEstadoCambios(){
		//... genera reporte de estadoCambios situacion Financiera en PDF
		$fechaGestion= $_POST['fechaDeGestion']; 		//... lee fechaGestion ...
		$anhoGestion=substr($fechaGestion,0,4);		//... asigna anho gestion ...			
		$mesGestion=substr($fechaGestion,4,2);		//... asigna mes gestion ...
		...
		if($permisoUserName!='superuser' && $permisoUserName!='developer' && $permisoMenu!='contabilidad'){  //... valida permiso de userName y de menu...
			...
			$sql="SELECT cuenta,descripcion,debeacumulado,haberacumulado,nivel FROM contaplandectas WHERE nivel<='2' AND cuenta<='39999999' AND(debeacumulado!=0.00 || haberacumulado!=0.00) ";
			
			$registros = $this->db->query($sql);
			$contador= $registros->num_rows; //...contador de registros que satisfacen la consulta ..
			
			if($contador==0){
				$datos['mensaje']='No hay registros seleccionados para el ESTADO DE CAMBIOS SITUACION FINANCIERA';
				$this->load->view('header');
				$this->load->view('mensaje',$datos );
				$this->load->view('footer');
			}else{
...
*/
				...
				$this->pdf->Cell(20,5,'Gerente general','',0,'C',0);
					...
					$datos['documento']="pdfsArchivos/contabilidad/balanceGeneral.pdf";	
					$datos['titulo']=' ESTADO CAMBIOS SITUACION FINANCIERA período de gestión: '.substr($fechaGestion,0,4).'-'.substr($fechaGestion,4,2);	// ... titulo ...
	...
	} //... fin funcion: generarReporteEstadoCambios Situacion Financiera ...
