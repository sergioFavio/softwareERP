...
function blancoPdf(posicionFila){
    
     var codigoProducto= $("#codigo_"+posicionFila).val();
    
	$.ajax({
      url: "<?=base_url()?>produccion/blancoPdf",
...
<div class="jumbotron" id="cuerpoCabecera" >	<!--cuerpoCrudMaterial-->
		<?php 
			if($buscarPor=="codProducto"){
				$funcionBuscar='buscarBlanco';
			}else{
				$funcionBuscar='buscarBlancoPorDescripcion';
			}
		?>
			...
			<div class="row">
				...
				<div class="col-xs-4">
			    	<div class="input-group input-group-sm">
			    		<input type="text" class="form-control input-sm" id="inputBuscarPatron" name="inputBuscarPatron" value='<?= $consultaBlanco ?>' placeholder="buscar por <?= $buscarPor ?>" data-toggle='tooltip' title="ingresar <?= $buscarPor ?>">
			   	...
			   	<div class="col-xs-3"> 
					<span  id="titulo" class="label label-success"> Ver Plantilla de Productos Blancos</span>
			   	</div>
...
<div class="jumbotron" id="cuerpoDetalle" >
	...
	<table width="99%" class="table table-striped table-bordered table-condensed" id="tabla1">   		
		<thead>
 		...
 		<tbody>	
			<?php  $posicionFila=-1; ?>			
	        <?php foreach($listaBlanco as $blanco):?>
				<tr class='letraDetalle'>
				
					<?php  $posicionFila=$posicionFila+1;  //...posicionFila
					
					$codigoProducto=$blanco->codProducto;
								
					 echo"<td style='width:60px;'><input type='text' id='codigo_".$posicionFila."' name='codigo_".$posicionFila."' value='".$blanco->codProducto."' readonly='readonly' style='border:none; width:60px;text-align:center;' /></td>";
							 
					 echo"<td style='width: 260px;'><input type='text' id='producto_".$posicionFila."' name='producto_".$posicionFila."' value='".$blanco->descripcion."' readonly='readonly' style='border:none; width:260px;' /></td>";
						
					 echo"<td style='width:220px;'><input type='text' id='medida_".$posicionFila."' name='medida_".$posicionFila."' value='".$blanco->medidas."' readonly='readonly' style='border:none; width:220px;text-align:center;' /></td>";

					 echo"<td style='width:60px;'><input type='text' id='unidad_".$posicionFila."' name='unidad_".$posicionFila."' value='".$blanco->unidad."' readonly='readonly' style='border:none; width:60px;' /></td>";
												
					 echo"<td style='width:50px;background-color:#b9e9ec;align=left;'><a href='#' onClick='blancoPdf($posicionFila);'><span class='glyphicon glyphicon-print'></span> PDF</a></td>";
					 	
					 if($permisoUserName=='superuser'){
					 	echo"<td style='width:65px;background-color:#a5d4da;align=left;'><a href='#' data-title='Eliminar plantilla producto blanco' data-item-id='".$blanco->codProducto."' data-cli='".$blanco->descripcion."' data-toggle='modal' data-target='#borrarModal'><span class='glyphicon glyphicon-trash'></span> Eliminar</a></td>"; 
					 }else{
...
<div class="modal fade" id="borrarModal" tabindex="-1" role="dialog" aria-labelledby="borrarModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <form class="form-horizontal" data-async data-target="#rating-modal" action="<?=base_url()?>produccion/eliminarPlantillaProductoBlanco" method="POST">
        ¿ Esta seguro de eliminar la plantilla producto blanco con código <span id="showCodigo" style="font-weight : bold;"></span> de <span id="showCliente" style="font-weight : bold;"></span> ?
