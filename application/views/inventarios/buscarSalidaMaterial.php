...
function buscarSalida(){
	...
	if($("#inputNumero").val()=="" ){
			alert("¡¡¡ E R R O R !!! ... El contenido de SALIDA No. está vacío, seleccione una salida");	
	}
	else{
...
<div class="jumbotron" id="cuerpoSalida">		
   ...
   <form class="form-horizontal" method="post" action="<?=base_url()?>materiales/modificarSalida" id="form_" name="form_" >
...
<!-- ... inicio  lightbox cabecerasalida... -->
  ...
  <div class="modal-content">
	...
	<div class="modal-body">
		...
		<table  cellspacing="0" cellpadding="0" border="0" class="display" id="tabla1">
			...
			<tbody>			
                <?php foreach($cabeceraSalidas as $cabeceraSalida):?>
                    <tr class='letraDetalleLightBox'>
                        <td style='width:80px;'> <?= $cabeceraSalida["numero"] ?></td>
                        <td style='width:100px;'> <?= fechaMysqlParaLatina($cabeceraSalida["fecha"])?></td>
                        <td style='width:100px;'> <?= $cabeceraSalida["numOrden"]?></td>
   						<td style='width:200px;' ><?= $cabeceraSalida['glosa']  ?></td>
                    </tr>
                <?php endforeach ?>
			</tbody>
		</table>
