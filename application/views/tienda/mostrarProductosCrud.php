...
<style type="text/css" >
	...
	#cuerpoCabecera{margin:0 auto; padding:0; width:860px; height:105px;}
	#cuerpoDetalle{margin:0 auto; padding:0; width:860px; height:410px;}
	#cuerpoPaginacion{margin:0 auto;padding:0; width:860px; height:60px;}
...
$(document).ready(function() {
	...
	*/
	...
	$('#borrarModal').on('show.bs.modal', function(e) {  
		...
		$(e.currentTarget).find('#showDescripcion').html(descripcion);
	...
	$('#editarModal').on('show.bs.modal', function(e) {  
		...
		var id = $(e.relatedTarget).data('item-id'); 
		var descripcion = $(e.relatedTarget).data('descripcion');
		var medidas = $(e.relatedTarget).data('medidas');
		...
		var title = $(e.relatedTarget).data('title');
		//aca asignamos a un hidden dentro del form que esta en el modal
	    $(e.currentTarget).find('input[name="inputCodigoM"]').val(id);
		$(e.currentTarget).find('input[name="inputDescripcionM"]').val(descripcion);
		$(e.currentTarget).find('input[name="inputMedidasM"]').val(medidas);
	...
	$('#editarModal').on("click", 'input[type="submit"], button[type="submit"]', function() {       
		...
		var idele3=$(form).find('input[name="inputMedidasM"]').val();
		var idele4=$(form).find('input[name="inputExistenciaM"]').val();
		var idele5=$(form).find('input[name="inputUnidadM"]').val();
		var idele6=$(form).find('input[name="inputPrecioVentaM"]').val();
...
function validarCodigoNoRepetido(numero){  
		...
		var codProducto= numero;	
		$.ajax({
	    	url: "validarCodigoProductoCrud",  
	        type:"POST",
	        data:{nombreTabla:'productosfabrica',campo:'idProd',patron:codProducto},
	        dataType: "json",
	        success: function(data){     
	     	   codExiste= data;	   
	     	   if(codExiste==true){
...
function validarPrecioVenta(numero){	
	var cantidad=parseFloat( numero ); // convierte de string to number 
	//if (!/^([0-9])*$/.test(numero))  // ... solo numeros enteros ...  
	if (!/^\d{1,7}(\.\d{1,3})?$/.test(numero)){  // ...hasta 7 digitos parte entera y hasta 3 parte decimal ...
		alert("El valor " + numero + " para PRECIO VENTA no es válido");
		$("#inputPrecioVenta").val("");   // borra celda 
	}
	    		
}   // fin ... validarPrecioUnidad ...

function validarPrecioVentaM(numero){	
	var cantidad=parseFloat( numero ); // convierte de string to number 
	//if (!/^([0-9])*$/.test(numero))  // ... solo numeros enteros ...  
	if (!/^\d{1,7}(\.\d{1,3})?$/.test(numero)){  // ...hasta 7 digitos parte entera y hasta 3 parte decimal ...
		alert("El valor " + numero + " para PRECIO VENTA no es válido");
		$("#inputPrecioVentaM").val("");   // borra celda 
	}
	    		
}   // fin ... validarPrecioVenta ...
	    ...
	    <form class="form-horizontal" method="post" action="<?=base_url()?>tienda/<?= $buscaPor ?>" id="formBuscarRegistro_" name="formBuscarRegistro_" >
		   ...
		   <div class="row">
			   	...
			   	<div class="col-xs-2 col-md-2"> 
			    	<button type="button"  class="btn btn-success btn-sm" id="openLightBox" title='Crea registro de nuevo producto'><span class="glyphicon glyphicon-plus-sign"></span> Añadir producto</button>
			    </div>
	...
	<table width="99%" class="table table-striped table-bordered table-condensed" id="tabla1">   		
		<thead>
    		<tr style="background-color: #b9e9ec;" class='letraDetalle'>
				...
				<th style="width: 372px;">Producto</th>
				<th style="width: 75px;">Existencia</th>
				<th style="width: 50px;">Unidad</th>
				<th style="width: 75px;">P. Venta Bs.</th>
				<th style="width: 75px;">Stock Mínimo</th>
	        ...
	        <?php foreach($listaProducto as $producto):?>
				<tr class='letraDetalle'>
					...
					<?php  $posicionFila=$posicionFila+1;  //...posicionFila
					 ...
					 echo"<td style='width:50px;'><input type='text' id='idItem_".$posicionFila."' name='idItem_".$posicionFila."' value='".$producto->idProd."' readonly='readonly' style='border:none; width:50px;' /></td>";
						
					 echo"<td style='width: 360px;'><input type='text' id='item_".$posicionFila."' name='item_".$posicionFila."' value='".$producto->nombreProd."' readonly='readonly' style='border:none; width:360px;' data-toggle='tooltip' title='$producto->medidas' /></td>";
							 
					 echo"<td style='width: 65px;'><input type='text' class='letraNumero' id='existItem_".$posicionFila."' name='existItem_".$posicionFila."' value='".number_format($producto->existencia,2)."' readonly='readonly' style='border:none; width:65px;' /></td>";
						
					 echo"<td style='width: 40px;'><input type='text' id='unidadItem_".$posicionFila."' name='unidadItem_".$posicionFila."' value='".$producto->unidad."' readonly='readonly' style='border:none; width:40px;' /></td>";
						
					 echo"<td style='width: 65px;'><input type='text' class='letraNumero' id='precioItem_".$posicionFila."' name='precioItem_".$posicionFila."' value='".number_format($producto->precioVenta,2)."' readonly='readonly' style='border:none; width:65px;' /></td>";
							
					 echo"<td style='width: 60px;'><input type='text' class='letraNumero' id='stockMinItem_".$posicionFila."' name='stockMinItem_".$posicionFila."' value='".number_format($producto->stockMinimo,2)."' readonly='readonly' style='border:none; width:60px;' /></td>";
						
					 echo"<td style='width:70px;background-color:#b9e9ec;' width='40' align='left'>
					 <a href='#' data-title='Editar registro' data-item-id='".$producto->idProd."' data-descripcion='".$producto->nombreProd."' data-medidas='".$producto->medidas."' data-existencia='".$producto->existencia."' 
					 data-unidad='".$producto->unidad."' data-precio='".$producto->precioVenta."' data-stock='".$producto->stockMinimo."' data-toggle='modal' data-target='#editarModal'><span class='glyphicon glyphicon-pencil'></span> Editar</a></td>";
				
					 echo"<td style='width:70px;background-color:#a5d4da;' width='40' align='left'><a href='#' data-title='Eliminar registro' data-item-id='".$producto->idProd."' data-descripcion='".$producto->nombreProd."' data-toggle='modal' data-target='#borrarModal'><span class='glyphicon glyphicon-trash'></span> Eliminar</a></td>"; 
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
      	 ...
      	 <form class="form-horizontal" method="post" action="<?=base_url()?>tienda/grabarNuevoProductoCrud" id="formGrabarNuevoRegistro_" name="formGrabarNuevoRegistro_" >
        	...
        	<div class="input-group input-group-sm">
			  ...
			  <input type="text" class="form-control input-sm" id="inputDescripcion" name="inputDescripcion" placeholder="nombre del material &hellip;">
			</div>
			
			<div style="height:5px;"></div>
        	
        	<div class="input-group input-group-sm">
			  <span class="input-group-addon">Medidas: </span>
			  <input type="text" class="form-control input-sm" id="inputMedidas" name="inputMedidas" placeholder="medidas &hellip;">
			</div>
			...
			<div class="input-group input-group-sm">
			  <span class="input-group-addon">Precio Venta Bs.: </span>
			  <input type="text" class="form-control input-sm" id="inputPrecioVenta" name="inputPrecioVenta" placeholder="precio venta &hellip;" onChange='validarPrecioVenta(this.value);'>
			</div>
...
<div class="modal fade" id="borrarModal" tabindex="-1" role="dialog" aria-labelledby="borrarModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
	  <form class="form-horizontal" data-async data-target="#rating-modal" action="<?=base_url()?>tienda/eliminarProductoCrud" method="POST">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
      	 ...
      	 <form class="form-horizontal"   data-async data-target="#rating-modal" action="<?=base_url()?>tienda/actualizarProductoCrud" id="formEditarRegistro_" name="formEditarRegistro_" method="POST">
      	
        	<div class="input-group input-group-sm">
			  <span class="input-group-addon">Código: </span>
			  	<input type='text' class='form-control input-sm' id='inputCodigoM' name='inputCodigoM' value='' readonly='readonly' placeholder='codigo&hellip;'  > 
			</div>
        	...
        	<div class="input-group input-group-sm">
			  <span class="input-group-addon">Producto: </span>
			  <input type="text" class="form-control input-sm" id="inputDescripcionM" name="inputDescripcionM" value='' placeholder="nombre del producto &hellip;">
			</div>
			...
			<div class="input-group input-group-sm">
			  <span class="input-group-addon">Medidas: </span>
			  <input type="text" class="form-control input-sm" id="inputMedidasM" name="inputMedidasM" value='' placeholder="medidas &hellip;">
			</div>
			...
			<div class="input-group input-group-sm">
			  <span class="input-group-addon">Precio Venta Bs.: </span>
			  <input type="text" class="form-control input-sm" id="inputPrecioVentaM" name="inputPrecioVentaM" value='' placeholder="precio venta &hellip;" onChange='validarPrecioVentaM(this.value);'>
			</div>
