...
function remisionPdf(nRemision){
    var remision= nRemision;
  
	$.ajax({
      url: "<?=base_url()?>tienda/remisionPdfCrud",

      type: "POST",
      data: {numeRemision: remision},
...
<div class="jumbotron" id="cuerpoCabecera" >	<!--cuerpoCrudMaterial-->
		<?php 
			if($buscarPor=="remision"){
				$funcionBuscar='buscarRemision';
			}else{
				$funcionBuscar='buscarRemisionPorCliente';
			}
		?>
			...
			<div class="row">
				...
				<div class="col-xs-3">
			    	<div class="input-group input-group-sm">
			    		<input type="text" class="form-control input-sm" id="inputBuscarPatron" name="inputBuscarPatron" value='<?= $consultaRemision ?>' placeholder="buscar por <?= $buscarPor ?>" data-toggle='tooltip' title="ingresar número de remisión sin ' / ' ">
			    ...
			    <div class="col-xs-1"> 
			   	...
			   	<div class="col-xs-3"> 
					<span  id="titulo" class="label label-success"> V e r  &nbsp;&nbsp; N o t a s &nbsp;&nbsp; d e &nbsp;&nbsp;  R e m i s i &oacute; n</span>
			   	</div>
	...
	<table width="99%" class="table table-striped table-bordered table-condensed" id="tabla1">   		
		<thead>
    		<tr style="background-color: #b9e9ec;" class='letraDetalle'>
    			<th style="width: 80px;">&nbsp;&nbsp;&nbsp;Remisión</th>
 		...
 		<tbody>	
			<?php  $posicionFila=-1; ?>			
	        <?php foreach($listaRemision as $remision):?>
				<tr class='letraDetalle'>
				
					<?php  $posicionFila=$posicionFila+1;  //...posicionFila
		
					$numeroRemision = $remision->remision;
					$local = $remision->local;
					$secuenciaRemision=substr($numeroRemision,0,4);
									
					if(strlen($numeroRemision)==6){						//...si es nota de entrega de Fabrica ... 
						$anhoSistema=substr($numeroRemision,4,2);
					}else{												//...si es nota de entrega de Tienda ... 
						$anhoSistema=substr($numeroRemision,4,4);
					}
					
			  		$numeRemisionAux=$secuenciaRemision.'/'.$anhoSistema;
								
					 echo"<td style='width:60px;'><input type='text' id='idPedido_".$posicionFila."' name='idPedido_".$posicionFila."' value='".$numeRemisionAux."' readonly='readonly' style='border:none; width:60px;text-align:center;' /></td>";
						
					 echo"<td style='width: 60px;'><input type='text' id='fechaEntrega_".$posicionFila."' name='fechaEntrega_".$posicionFila."' value='".fechaMysqlParaLatina($remision->fecha)."' readonly='readonly' style='border:none; width:60px;' /></td>";
							 
					 echo"<td style='width: 200px;'><input type='text' id='contactoEmpresa_".$posicionFila."' name='contactoEmpresa_".$posicionFila."' value='".$remision->cliente."' readonly='readonly' style='border:none; width:200px;' /></td>";
						
					 echo"<td style='width: 100px;'><input type='text' id='telCel_".$posicionFila."' name='telCel_".$posicionFila."' value='".$remision->telefono."' readonly='readonly' style='border:none; width:100px;text-align:center;' /></td>";

					 echo"<td style='width: 180px;'><input type='text' id='correo_".$posicionFila."' name='correo_".$posicionFila."' value='".$remision->pedido."' readonly='readonly' style='border:none; width:180px;' /></td>";
												
					 echo"<td style='width:50px;background-color:#b9e9ec;align=left;'><a href='#' onClick='remisionPdf($numeroRemision);'><span class='glyphicon glyphicon-print'></span> PDF</a></td>";
					 	
					 if($permisoUserName=='superuser'){
					 	echo"<td style='width:65px;background-color:#a5d4da;align=left;'><a href='#' data-title='Eliminar nota de remisión' data-item-id='".$remision->remision."' data-cli='".$remision->cliente."' data-toggle='modal' data-target='#borrarModal'><span class='glyphicon glyphicon-trash'></span> Eliminar</a></td>"; 
					 }else{
...
<!-- ...  lightbox borrar nota de remision ... -->
...
<div class="modal fade" id="borrarModal" tabindex="-1" role="dialog" aria-labelledby="borrarModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <form class="form-horizontal" data-async data-target="#rating-modal" action="<?=base_url()?>tienda/eliminarNotaRemision" method="POST">
        ¿ Esta seguro de eliminar la nota de remisión <span id="showCodigo" style="font-weight : bold;"></span> de <span id="showCliente" style="font-weight : bold;"></span> ?
...
<!-- ... fin  lightbox borrar nota de remision ... -->


<!-- ... inicio  lightbox remisionPdf... -->
